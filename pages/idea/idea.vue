<template>
	<view>
		<!-- 导航 -->
		<uniNavBar :status-bar="true" background-color="#4378BE" color="#ffffff" title="我的提案" fixed @clickRight="filtrate">
			<template slot="left">
...
<script>
	...
	export default {
		...
		methods: {
			...
			viewComments(id) {
				uni.redirectTo({
					url: `../proposal-detail/proposal-detail?flag=1&proposalId=${id}&moduleId=5`
				})
			},
			...
			proposalDetail(id) {
				uni.redirectTo({
					url: `../proposal-detail/proposal-detail?proposalId=${id}&moduleId=5`
				})
			},
			getData() {
				if (this.isLike) {
